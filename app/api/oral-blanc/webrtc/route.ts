...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Oral Blanc WebRTC Proxy] Negotiating SDP with OpenAI...');
    ...
    if (!response.ok) {
      ...
      console.error('[Oral Blanc WebRTC Proxy] OpenAI error:', response.status, errorText);
      return NextResponse.json({ 
    ...
    console.log('[Oral Blanc WebRTC Proxy] SDP negotiation successful');
    ...
    console.error('[Oral Blanc WebRTC Proxy] Error:', error);
    return NextResponse.json({ 
